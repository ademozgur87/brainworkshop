...
IMAGE_SETS = ['emotion-comic-female']
    ...
    if not (CLINICAL_MODE and configpath == 'config.ini'):
        ...
        try:
            ...
            if configpath != 'config.ini':
                quit_with_error(_(u'Config (Ayar) dosyası yüklenemedi: %s') %
...
sound = sounds['harfler'] # is this obsolete yet?
...
class Mode:
    def __init__(self):
        ...
        self.long_mode_names =  {2:_(u'İkili (Dual)'),
                                 3:_(u'Konum, Renk, Ses'),
                                 4:_(u'Dual Kombinasyon'),
                                 5:_(u'Üçlü (Triple) Kombinasyon'),
                                 6:_(u'Dörtlü (Quad) Kombinasyon'), 
                                 7:_(u'Aritmetik'),
                                 8:_(u'Dual Aritmetik'),
                                 9:_(u'Triple Aritmetik'),
                                 10:_(u'Konum'),
                                 11:_(u'Ses'),
                                 12:_(u'Tri Kombinasyon (Renk)'),
                                 20:_(u'Konum, Renk'),
                                 21:_(u'Konum, Resim'),
                                 22:_(u'Renk, Ses'),
                                 23:_(u'Resim, Ses'),
                                 24:_(u'Renk, Resim'),
                                 25:_(u'Konum, Renk, Resim'),
                                 26:_(u'Konum, Resim, Ses'),
                                 27:_(u'Renk, Resim, Ses'),
                                 28:_(u'Dörtlü (Quad)'),
                                 100:_(u'Ses, Ses2'),
                                 101:_(u'Konum, Ses, Ses2'),
                                 102:_(u'Renk, Ses, Ses2'),
                                 103:_(u'Resim, Ses, Ses2'),
                                 104:_(u'Konum, Renk, Ses, Ses2'),
                                 105:_(u'Konum, Resim, Ses, Ses2'),
                                 106:_(u'Renk, Resim, Ses, Ses2'),
                                 107:_(u'Beşli (Pentuple)')
                                 }
...
class Graph:
    ...
    def create_batch(self):
        ...
        pyglet.text.Label(
            _(u'G: Ana Pencereye Dön\n\nN: Sıradaki Oyun Türü'),
            batch=self.batch,
        ...
        dates.sort()
        if len(dates) < 2:
            pyglet.text.Label(_(u'Yetersiz veri: 2 günlük veri gerekmektedir'),
                batch=self.batch,
        ...
        for index in range(len(dates)):
            ...
            if not index % (skip_x + 1):
                pyglet.text.Label(datestring, multiline=True, width=12,
                    batch=self.batch,
                    font_size=8, bold=False, color=cfg.COLOR_TEXT,
                    x=x, y=bottom - 15,
        ...
        while y_marking <= ymax:
            ...
            pyglet.text.Label(str(round(y_marking, 2)),
                batch=self.batch,
                font_size = 10, bold=False, color=cfg.COLOR_TEXT,
                x = left - 30, y = y + 1,
                anchor_x = 'center', anchor_y = 'center')
            self.batch.add(2, GL_LINES, 
                pyglet.graphics.OrderedGroup(order=0), ('v2i', (
        ...
        labelstrings = {'position1':_(u'Konum: ')  , 'position2':_(u'Konum 2: '), 
                        'position3':_(u'Konum 3: '), 'position4':_(u'Konum 4: '),
                        'vis1':_(u'Renk/Görsel 1: '), 'vis2':_(u'Renk/Görsel 2: '), 
                        'vis3':_(u'Renk/Görsel 3: '), 'vis4':_(u'Renk/Görsel 4: '),
                        'visvis':_(u'Görsel & n-görsel: '), 'visaudio':_(u'Görsel & n-ses: '), 
                        'audiovis':_(u'Ses & n-görsel: '), 'audio':_(u'Ses: '),
                        'color':_(u'Renk: '), 'image':_(u'Görsel: '),
                        'arithmetic':_(u'Aritmetik: '), 'audio2':_(u'Ses2: ')}       
        str_list = [_(u'Son 50 deneme:   ')]
        ...
        pyglet.text.Label(''.join(str_list),
            batch=self.batch,
            font_size = 11, bold = False, color = cfg.COLOR_TEXT,
...
class Menu:
    ...
    def __init__(self, options, values=None, actions={}, names={}, title='', 
                 footnote = _(u'Esc: iptal et     Boşluk: seçeneği değiştir     Enter: uygula'), 
    ...
    #(Adm) Ceviri yapilmali mi?    
    def textify(self, x):
        if type(x) == bool:
            return x and _(u'Evet') or _(u'Hayır')
        return str(x)
    ...
    def choose(self, k, i): # override this method in subclasses
        print u"Yazılımımızın beta sürümünü tercih ettiğiniz için teşekkürler."
...
class MainMenu(Menu):
    def __init__(self):
        ...
        ops = [('game', _(u'Oyun türünü seçin'), GameSelect),
               ('sounds', _(u'Sesleri seçin'), SoundSelect),
               ('images', _(u'Resimleri seçin'), ImageSelect),
               ('user', _(u'Kullanıcı seçin'), UserScreen),
               ('graph', _(u'Günlük İlerleme Grafiği'), NotImplemented),
               ('help', _(u'Yardım / Eğitim'), NotImplemented),
               ('donate', _(u'Bağış'), NotImplemented)
               ('forum', _(u'Foruma / Mail Listesine gidin'), NotImplemented)]
...
class UserScreen(Menu):
    def __init__(self):
        ...
        self.users = users = [_("Yeni kullanici"), 'Blank line'] + get_users()   #New user
        Menu.__init__(self, options=users, 
                      #actions=dict([(user, choose_user) for user in users]),
                      title=_(u"Kullanıcı profilinizi seçiniz"),
                      choose_once=True,
    ...
    def choose(self, k, i):
        ...
        if newuser == _("Yeni kullanici"):
            textInput = TextInputScreen(_(u"Yeni kullanıcı adı giriniz (Türkçe karakter kullanmayınız):"), USER, callback=set_user, catch=' ')
        else:
            set_user(newuser)
...
class LanguageScreen(Menu):
    def __init__(self):
        ...
        Menu.__init__(self, options=languages,
                      title=_(u"Tercih ettiğiniz dili seçiniz"),
                      choose_once=True,
                      default=default)
    def save(self):
        self.select() 
...
class OptionsScreen(Menu):
    def __init__(self):
        ...
        options = cfg.keys()
        options.sort()
        Menu.__init__(self, options=options, values=cfg, title=_(u'Yapılandırma'))

# (Adm) Ceviri modaliteler icin names yazilabilir
class GameSelect(Menu):
    def __init__(self):
        modalities = ['position1', 'color', 'image', 'audio', 'audio2', 'arithmetic']
        options = modalities[:]
        names = dict([(m, _(u"%s kullan") % m) for m in modalities])
        names['position1'] = _(u"Konum kullan")
        ...
        names['combination'] = _(u'Kombinasyon N-Geri modu')  
        names['variable'] = _(u'Değişken N-Geri düzeyleri kullan')
        names['crab'] = _(u'Yengeç-geri modu (N uyarıcı setinin ters sıralanışı)')
        names['multi'] = _(u'Eşzamanlı görsel uyaran')
        names['multimode'] = _(u'Şuna göre farklılaşan eşzamanlı uyaran')
        names['selfpaced'] = _(u'Kendi hızında modu')
        names['interference'] = _(u'Müdahale (hileli uyarıcı oluşturma)')
        vals = dict([[op, None] for op in options])
        ...
        Menu.__init__(self, options, vals, names=names, title=_(u'Oyun türünü seçiniz'))        
        ...
        try:
            if self.newmode:
                self.modelabel.text = mode.long_mode_names[self.newmode] + \
                    (self.values['variable'] and ' V.' or '') + ' N-Geri'
            else:
                self.modelabel.text = _(u"Geçersiz bir tür seçildi.")
...
class ImageSelect(Menu):
    def __init__(self):
        ...
        options.sort()
        vals = self.new_sets
        Menu.__init__(self, options, vals, title=_(u'Resim n-geri görevlerinde kullanmak için resimleri seçin.'))
    ...
    def __init__(self):
        ...
        for op in options:
            if op.startswith('1') or op.startswith('2'):
                names[op] = _(u"Kanal '%s' için '%s' ses setini kullan") % (op[0], op[1:])
            elif 'CHANNEL_AUDIO' in op:
                names[op] = 'Kanal %i :' % (op[-1]=='2' and 2 or 1)
        Menu.__init__(self, options, vals, {}, names, title=_(u'Ses n-geri görevleri için ses setlerini seçin.'))
...
class UpdateLabel:
    ...
    def update(self):
        if not mode.started and update_available:
            str_list = []
            str_list.append(_(u'Yeni bir güncelleme var ('))
            str_list.append(str(update_version))
            str_list.append(_(u'). web sayfasını açmak için W tuşuna basınız'))
...
class JaeggiWarningLabel:
    ...
    def show(self):
        ...
        self.label.text = _(u'Ek modlara erişmek için Jaeggi modunu kapatınız.')
    ...
    def update(self):
        ...
        if mode.started:
            ...
            if not mode.hide_text:
                str_list.append(_(u'P: Duraklat / Devam et\n'))
                str_list.append('\n')
                str_list.append(_(u'F8: Metinleri Gizle / Göster\n'))
                str_list.append('\n')                
                str_list.append(_(u'ESC: Oturumu İptal Et\n'))
        elif CLINICAL_MODE:
            self.label.y = window.height - 10
            str_list.append(_(u'ESC: Çıkış'))
        else:
            ...
            if 'morse' in cfg.AUDIO1_SETS or 'morse' in cfg.AUDIO2_SETS:
                str_list.append(_(u'J: Mors Alfabesine Başvur\n'))
                str_list.append('\n')
            str_list.append(_(u'H: Yardım / Eğitim\n'))
            str_list.append('\n')
            if mode.manual:
                str_list.append(_(u'F1: N-Geriyi Azalt\n'))
                str_list.append(_(u'F2: N-Geriyi Artır\n'))
                str_list.append('\n')
                str_list.append(_(u'F3: Denemeleri Azalt\n'))
                str_list.append(_(u'F4: Denemeleri Artır\n'))
                str_list.append('\n')
            if mode.manual:
                str_list.append(_(u'F5: Hızı Azalt\n'))
                str_list.append(_(u'F6: Hızı Artır\n'))
                str_list.append('\n')
            str_list.append(_(u'C: Oyun Türünü Seç\n'))
            str_list.append(_(u'S: Sesleri Seç\n'))
            str_list.append(_(u'I:  Resimleri Seç\n'))
            if mode.manual:
                str_list.append(_(u'M: Standart Mod\n'))
            else:
                str_list.append(_(u'M: Manuel Mod\n'))
            str_list.append(_(u'D: Bağış Yapın\n'))
            str_list.append('\n')
            str_list.append(_(u'G: Günlük İlerleme Grafiği\n'))
            str_list.append('\n')
            str_list.append(_(u'W: Beyin Atölyesi Web Sitesi\n'))
            if cfg.WINDOW_FULLSCREEN:
                str_list.append(_(u'E: Göz Sıçrama (Saccadic) Egzersizi\n'))
            str_list.append('\n')
            str_list.append(_(u'ESC: Çıkış\n'))
...
class TitleKeysLabel:
    def __init__(self):
        ...
        if not (cfg.JAEGGI_MODE or CLINICAL_MODE):
            str_list.append(_(u'C: Oyun türünü seçin\n'))
            str_list.append(_(u'S: Sesleri seçin\n'))
            str_list.append(_(u'I: Resimleri seçin\n'))
        if not CLINICAL_MODE:
            str_list.append(_(u'U: Kullanıcı seçin\n'))
            str_list.append(_(u'G: Günlük ilerleme grafiği\n'))
        str_list.append(_(u'H: Yardım / Eğitim\n'))
        if not CLINICAL_MODE:
            str_list.append(_(u'D: Bağış yapın\n'))
            str_list.append(_(u'F: Foruma / Mail listesine gidin\n'))
            str_list.append(_(u'O: Ayar dosyasını düzenleyin'))
        ...
        self.space = pyglet.text.Label(
            _(u'Beyin Atölyesine başlamak için BOŞLUK tuşuna basınız'),
...
class LogoUpperLabel:
    def __init__(self):
        self.label = pyglet.text.Label(
            u'Beyin', # I think we shouldn't translate the program name.  Yes?
            font_size=11, bold = True,
            color=cfg.COLOR_TEXT,
...
class PausedLabel:
    ...
    def update(self):
        if mode.paused:
            self.label.text = u'Duraklatıldı'
        else:
            self.label.text = ''
...
class CongratsLabel:
    ...
    def update(self, show=False, advance=False, fallback=False, awesome=False, great=False, good=False, perfect = False):
        ...
        if show and not CLINICAL_MODE and cfg.USE_SESSION_FEEDBACK:
            if perfect: str_list.append(_(u'Mükemmel puan! '))
            elif awesome: str_list.append(_(u'Müthiş puan! '))
            elif great: str_list.append(_(u'Harika puan! '))
            elif good: str_list.append(_(u'Fena değil! '))
            else: str_list.append(_(u'Devam et. İyi gidiyorsun! '))
        if advance:
            str_list.append(_(u'N-Geri artırıldı'))
        elif fallback:
            str_list.append(_(u'N-Geri azaltıldı'))
        self.label.text = ''.join(str_list)
...
class FeedbackLabel:
	...
	if total == 2 and not cfg.JAEGGI_MODE and cfg.ENABLE_MOUSE:
	    if pos == 0:
		self.mousetext = u"Sol tık veya"
	    if pos == 1:
		self.mousetext = u"Sağ tık veya"
	else:
	    self.mousetext = ""
...
class ThresholdLabel:
    ...
    def update(self):
        if mode.started or mode.manual or CLINICAL_MODE:
            ...
            self.label.text = _(u'Eşik Değeri:\nDüzeyi artır: \u2265 %i%%\nDüzeyi düşür: < %i%%') % \
...
class SpaceLabel:
    ...
    def update(self):
        if mode.started:
            ...
            str_list = []
            str_list.append(_(u'Oturum #'))
            str_list.append(str(mode.session_number + 1))
            str_list.append(_(u', başlamak için BOŞLUK tuşuna basın '))
            str_list.append(': ')
            str_list.append(mode.long_mode_names[mode.mode] + ' ')
    ...
    def update(self, skip=False):
        ...
        if not CLINICAL_MODE:
            ...
            if 'arithmetic' in mods:
                str_list += ["%s:%i-%i%s" % (_(u"Aritmetik"), rights['arithmetic'], wrongs['arithmetic'], sep)]
        ...
        if cfg.JAEGGI_SCORING:
            ...
            if not CLINICAL_MODE:
                str_list += [_(u'En düşük puan: %i%%') % percent]
        else:
            percent = calc_percent(right, wrong)
            str_list += [_(u'Puan: %i%%') % percent]
...
class ChartTitleLabel:
    ...
    def update(self):
        if mode.started:
            ...
            self.label.text = _(u'Bugünün son 20 puanı:')
    ...
    def update(self):
        if mode.started or CLINICAL_MODE:
            ...
            if sessions:
                ...
                average = 0.
            self.label.text = _(u"%s-NG ortalama: %1.2f") % (mode.short_mode_names[mode.mode], average)
...
class TodayLabel:
    ...
    def update(self):
        if mode.started:
            ...
            self.labelTitle.text = _(u"Bügünkü %i oturum %i dk %i sn sürdü\
			    Son 24 saatteki %i oturum  %i dk %i sn sürdü" % (stats.sessions_today, stats.time_today//60, stats.time_today%60, stats.sessions_thours, stats.time_thours//60, stats.time_thours%60))
...
class TrialsRemainingLabel:
    ...
    def update(self):
        if (not mode.started) or mode.hide_text:
            ...
            self.label.text = _(u'Kalan %i') % (mode.num_trials_total - mode.trial_number)
    ...
    def parse_statsfile(self):
        ...
        if os.path.isfile(os.path.join(get_data_dir(), cfg.STATSFILE)):
            try:
                ...
                quit_with_error(_(u'stats dosyası işlenirken Hata alındı\n%s') %
                                os.path.join(get_data_dir(), cfg.STATSFILE),
                                _(u'\nLütfen stats dosyasını onarın, silin veya yeniden adlandırın.'),
                                quit=False)
    ...
    def submit_session(self, percent, category_percents):
        ...
        if ATTEMPT_TO_SAVE_STATS:
            try:
                ...
                quit_with_error(_(u'stats dosyası yazılırken hata alındı\n%s') % 
                                os.path.join(get_data_dir(), cfg.STATSFILE),
                                _(u'\nLütfen dosya ve dizin izinlerini kontrol edin.'))
